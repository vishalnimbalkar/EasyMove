import { booking, getBookingDetails,cancelBooking, getAllBookingDetails, getBookingDetailsForDriver, completeBooking, assignDriver } from "../services/bookingServices.js";
...
export const booking_request = async (req, res) => {
  ...
  export const assignDriverController = async (req, res)=>{
    ...
    try {
      const response = await assignDriver(booking_id,driver_id);
      if (response.success) {
        res.status(200).json(response);
      } else {
        res.status(400).json(response);
